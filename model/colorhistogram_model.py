...
class ColorHistogram_Model(torch.nn.Module):
    def name(self):
    ...
    def forward(self):
        ...
        # print(self.tar.dtype)
        inp_H_list=[]
        tar_H_list = []
        ## HEN ##
        for i in range(0, self.opt.batchSize):
            hist_inp_ab = self.getHistogram2d_np(self.inp[i:i+1], self.hist_ab)
            hist_inp_l = self.getHistogram1d_np(self.inp[i:i+1], self.hist_l).repeat(1, 1, self.hist_ab, self.hist_ab)
            hist_inp = torch.cat((hist_inp_ab, hist_inp_l), 1)
            inp_H_list.append(hist_inp)
            hist_tar_ab = self.getHistogram2d_np(self.tar[i:i+1], self.hist_ab)
            hist_tar_l = self.getHistogram1d_np(self.tar[i:i+1], self.hist_l).repeat(1, 1, self.hist_ab, self.hist_ab)
            hist_tar = torch.cat((hist_tar_ab, hist_tar_l), 1)
            tar_H_list.append(hist_tar)

        hist_inp = torch.cat(inp_H_list, dim=0)
        hist_tar = torch.cat(tar_H_list, dim=0)
        ...
        # print(self.inp.shape, hist_tar_l.shape, hist_tar_ab.shape)
        _, _, _, _, out = self.IRN(self.inp, self.inp_H, self.tar_H)

        self.inp = self.inp[:, :, self.pad:(self.inp.size(2) - 1 * self.pad),
                   self.pad:(self.inp.size(3) - 1 * self.pad)]
        # self.tar = self.tar[:, :, self.pad:(self.tar.size(2) - 2 * self.pad),
        #            self.pad:(self.tar.size(3) - 2 * self.pad)]
        self.out = out[:, :, self.pad:(out.size(2) - 1 * self.pad), self.pad:(out.size(3) - 1 * self.pad)]
        # print(self.out.shape, self.tar.shape,self.inp.shape)

    def get_current_losses(self):
        loss=self.MSE(self.out, self.tar)
        
        out_H_list=[]
        tar_H_list=[]
        for i in range(0, self.opt.batchSize):
            hist_inp_ab = self.getHistogram2d_np(self.out[i:i+1], self.hist_ab)
            hist_inp_l = self.getHistogram1d_np(self.out[i:i+1], self.hist_l).repeat(1, 1, self.hist_ab, self.hist_ab)
            hist_inp = torch.cat((hist_inp_ab, hist_inp_l), 1)
            out_H_list.append(hist_inp)
            hist_tar_ab = self.getHistogram2d_np(self.tar[i:i+1], self.hist_ab)
            hist_tar_l = self.getHistogram1d_np(self.tar[i:i+1], self.hist_l).repeat(1, 1, self.hist_ab, self.hist_ab)
            hist_tar = torch.cat((hist_tar_ab, hist_tar_l), 1)
            tar_H_list.append(hist_tar)
        hist_out = torch.cat(out_H_list, dim=0)
        hist_tar = torch.cat(tar_H_list, dim=0)
        loss_hist = self.MSE(hist_out, hist_tar)
        # print(loss_hist)
        # return loss
        return loss+loss_hist*1.5

    def save_networks(self,epoch,optimizer,scheduler):
        checkpoint = {
            'epoch': epoch,
            'IRN': self.IRN.state_dict(),
            'HEN': self.HEN.state_dict(),
    ...
    def test(self):
        with torch.no_grad():
            ...
            with torch.no_grad():
                ...
                print(hist_inp_ab.shape,self.hist_ab)
                # a=T.ToPILImage()(hist_inp_ab[0].float())
                # a.show()
                torch.cuda.empty_cache()
                hist_inp_l =  self.getHistogram1d_np(self.inp, self.hist_l).repeat(1,1,self.hist_ab,self.hist_ab)
                torch.cuda.empty_cache()
                print(hist_inp_l.shape)
                hist_inp = torch.cat((hist_inp_ab,hist_inp_l),1)
                print(hist_inp.shape)
                # a=T.ToPILImage()(hist_inp[0].float())
                # a.show()
                torch.cuda.empty_cache()
                ...
                torch.cuda.empty_cache()
                hist_inp_feat  = self.HEN(hist_inp)
                print(hist_inp_feat.shape)
                torch.cuda.empty_cache()
                hist_tar_feat  = self.HEN(hist_tar)
                torch.cuda.empty_cache()
                hist_inp_feat_tile =    hist_inp_feat.repeat(1,1,self.inp.size(2),self.inp.size(3))
                torch.cuda.empty_cache()
                print(hist_inp_feat_tile.shape)
                hist_tar_feat_tile =    hist_tar_feat.repeat(1,1,self.tar.size(2),self.tar.size(3))
                torch.cuda.empty_cache()
                self.final_result_tar = hist_tar_feat.repeat(1,1,self.inp.size(2),self.inp.size(3))
                torch.cuda.empty_cache()
