...
class Restormer_CNN_block(nn.Module):
    def __init__(self,in_dim,out_dim):
        ...
        self.GlobalFeature1 = GlobalFeatureExtraction(dim=out_dim, num_heads = 8)
        self.LocalFeature1 = LocalFeatureExtraction(dim=out_dim)
        # self.GlobalFeature2 = GlobalFeatureExtraction(dim=out_dim, num_heads = 8)
        # self.LocalFeature2 = LocalFeatureExtraction(dim=out_dim)
        # self.GlobalFeature3 = GlobalFeatureExtraction(dim=out_dim, num_heads = 8)
        # self.LocalFeature3 = LocalFeatureExtraction(dim=out_dim)
        self.FFN=nn.Conv2d(out_dim*2, out_dim,kernel_size=3,stride=1, padding=1, bias=False,padding_mode="reflect")          
    def forward(self, x):
        x=self.embed(x)
        x1=self.GlobalFeature1(x)
        x2=self.LocalFeature1(x)
        # x1=self.GlobalFeature2(x1)
        # x2=self.LocalFeature2(x2)
        # x1=self.GlobalFeature3(x1)
        # x2=self.LocalFeature3(x2)
...
class Ufuser(nn.Module):
    def __init__(self):
        ...
        channel=[8,16,32,32]
        self.V_en_1 = Restormer_CNN_block(64, channel[0])
        ...
        self.I_en_1 = Restormer_CNN_block(64, channel[0])
        ...
        self.last = nn.Sequential(
            nn.Conv2d(channel[0], 64, kernel_size=3, stride=1, padding=1,padding_mode="reflect"),
            nn.Sigmoid()
        )
