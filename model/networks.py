...
class HISTUnet3_Res(nn.Module): 
    ...
    def forward(self, input_img, hist1_enc, hist2_enc):
        ...
        out_img5 = F.interpolate(out_img5,size = (input_img.size(2), input_img.size(3)), mode = 'bilinear')
...
class UnetDec_deep1(nn.Module):
    ...
    def forward(self, input1, input2, enc1, enc2, target_size1, target_size2):
        input1=F.interpolate(input1,size= (target_size1, target_size2), mode = 'bilinear')
        enc1 = F.interpolate(enc1, size = (target_size1, target_size2), mode = 'bilinear')
        enc2 = F.interpolate(enc2, size = (target_size1, target_size2), mode = 'bilinear')
        input2=F.interpolate(input2,size= (target_size1, target_size2), mode = 'bilinear')
...
class UnetDec_deep2(nn.Module):
    ...
    def forward(self, input1, input2, enc1, enc2, target_size1, target_size2):
        ...
        enc2 = F.interpolate(enc2,  size = (target_size1, target_size2), mode = 'bilinear')
        input2=F.interpolate(input2,size = (target_size1, target_size2), mode = 'bilinear')
        # print(input1.shape,input2.shape,enc1.shape,enc2.shape)
        out = self.block(torch.cat([input1, input2, enc1, enc2],1))
...
class ENCResnetBlock(nn.Module):
    ...
    def forward(self, x, enc1, enc2): #64/64/64
        enc1 = F.interpolate(enc1,size = (x.size(2), x.size(3)), mode = 'bilinear')
        enc2 = F.interpolate(enc2,size = (x.size(2), x.size(3)), mode = 'bilinear')
